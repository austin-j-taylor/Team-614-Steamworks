...
public class RotateToVisionTarget extends Command {
	...
	boolean rotationDirection = false; // left for false, right for true
										// used if the robot loses sight of its
										// vision target
	...
	public RotateToVisionTarget(boolean rotationDirection) {
		requires(Robot.drivetrain);

		this.rotationDirection = rotationDirection;
		this.shouldRotateIfNoVision = false;
		this.usingGearCamera = false;
	...
	protected void initialize() {
		...
		Robot.drivetrain.getTurnController().setSetpoint(
				Robot.navX.getYaw() + angle + Constants.SHOOTER_CAMERA_OFFSET);
	}
	...
	protected void execute() {
		if (Robot.cameraIsActive) {
			...
			// // targetFound = Robot.gearCamera.getBoolean("targetFound",
			// false);
			...
			if (shouldRotateIfNoVision == true && targetFound == false) {
				// vision target not yet seen and the robot should act on this
				...
				// stores target rotation direction in case the robot loses
				// vision target
				...
				// } else { // shooter/ camera
				// Robot.drivetrain.getTurnController().setSetpoint(angle +
				// Constants.SHOOTER_CAMERA_OFFSET);
				// }
			}
			if (angle > -5 && angle < 5) {
				hitTarget = true;
